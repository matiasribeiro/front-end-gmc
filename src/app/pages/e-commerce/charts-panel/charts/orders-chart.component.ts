...
export class OrdersChartComponent implements AfterViewInit, OnDestroy, OnChanges {
  ...
  ngOnChanges(): void {
    if (this.option) {
      this.updateOrdersChartOptions(this.linhaChartData);
    }
  }
  ...
  updateOrdersChartOptions(linhaChart: LinhaChart) {

    //console.log("orders-chart ---- criar gráfico --- " + typeof(this.linhaChartData.dados.values));
    ...
    if(linhaChart !== undefined){

      //console.log("orders-chart ---- criar gráfico --- " + linhaChart.label);

      const series = this.getNewSeries(options.series, linhaChart.dados);
      const xAxis = this.getNewXAxis(options.xAxis, linhaChart.label);
